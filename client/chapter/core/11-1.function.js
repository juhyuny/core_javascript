...
const result = calcPrice(1000,400);
...
// 좋은 함수 작성 여건
// 1. 하나의 기능만을 수행해야 한다. 
// 2. 이름과 매개변수의 이름을 직관적으로 적어야한다. ( 주석을 덜 쓴다. -> 가독성 => 읽었을 떄 바로 기능을 알 수 있다. )
// 3. 재사용성이 좋아야한다. 
...
function rem(pxValue, base = 16){
  // let result = (parseFloat(pxValue) / base) + 'rem';
  // return result;
...
function getStyle(node, prop){
  if(typeof node === 'string') node = document.querySelecttor(node);
...
// setStyle(document.body,'background','hotpink');
setStyle('.first','background','hotpink');

// getStyle(document.body, 'fontSize'))

// console.log(setStyle('body','background-color', 'orange'));
